...
const COL_KIND = {
  ...
  value_before: 'real', value_after: 'real', value_delta: 'real',
};
...
async function main() {
  ...
  for (const ds of datasets) {
    ...
    contractRows += cN;
    amendRows += aN;
    process.stderr.write(`   ${cN.toLocaleString('en-US')} contracts, ${aN.toLocaleString('en-US')} amendments\n`);
  }

  cOut.end();
  aOut.end();
  await Promise.all([once(cOut, 'finish'), once(aOut, 'finish')]);
  process.stderr.write(
    `==> wrote ${contractRows.toLocaleString('en-US')} contracts + ${amendRows.toLocaleString('en-US')} amendments ` +
      `(largest stmt ${Math.max(cb.max(), ab.max()).toLocaleString('en-US')} bytes)\n`,
  );
  ...
  process.stderr.write(`\n==> done: ${contractRows} contracts, ${amendRows} amendments\n`);
}
