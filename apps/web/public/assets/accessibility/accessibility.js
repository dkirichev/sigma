...
const accessibility = async (opts) => {
    ...
    if (!parent || !main || !nav) {
        throw new Error(`ACCESSIBILITY-TOOL: Element not found for 1 or more selectors : ${parentSelector}, ${navSelector}, ${mainSelector}`);
    }
    ...
    const increaseFontSize = (e) => {
        ...
        document.querySelector('.a11y__item--increase').querySelector('span').textContent = body.classList.contains('font-resize')
    ...
    const buttonsData = [
        [
            ...
            Number(localStorage.getItem('accesibility.fontSize')) ? translations[lng]?.fontDown : translations[lng]?.fontUp,
            increaseFontSize,
        ],
    ...
    const controlPanelNav = ef(
        ...
        ef(
            ...
            ...buttonsData.map(([c, t, f]) => ef('li', { class: c, event: { type: 'click', callback: f } }, ef('button', { class: 'a11y__item__button'}, ef('span', {}, t)))),
            ef('li', { class: 'a11y__item a11y__item--credits' }, ef('p', {}, translations[lng].logoText ?? '© Information Services Jsc'))
        )
    );
    ...
    const leftContent = ef(
        ...
        ef(
            'ul',
            { class: 'innerUl' },
            ef('li', { 'data-target': navSelector, class: 'a11y-def-click' }, ef('a', { href: '#' }, translations[lng].toNav)),
            ef('li', { 'data-target': mainSelector, class: 'a11y-def-click' }, ef('a', { href: '#' }, translations[lng].toMain)),
            ef(
                'li',
                { class: 'ally-click' },
                ef('ul', {}, ...[...document.querySelectorAll(linkSelector)].map((e) => ef('li', {}, ef('a', { href: '#' }, e.textContent))))
            )
        )
    );
